...
class CodeAnalysisRequest(BaseModel):
    ...
    code: str = Field(
        ..., min_length=1, max_length=100000, description="Source code to analyze"
    )
    language: LanguageEnum = Field(
        default=LanguageEnum.PYTHON, description="Programming language of the code"
    )
...
class FeedbackRequest(BaseModel):
    ...
    correct_label: Optional[str] = Field(
        default=None, description="The correct vulnerability type if misclassified"
    )
...
class BatchAnalysisRequest(BaseModel):
    ...
    files: List[CodeAnalysisRequest] = Field(
        ..., min_length=1, max_length=50, description="List of code files to analyze"
    )
